...
export PEER0_ORG2_CA=${PWD}/crypto-config/peerOrganizations/braketester.br/peers/peer0.braketester.br/tls/ca.crt
...
setGlobals() {
  ...
  elif [ $USING_ORG == "braketester.br" ]; then
    export CORE_PEER_LOCALMSPID="BRAKETESTERMSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG2_CA
    export CORE_PEER_MSPCONFIGPATH=${PWD}/crypto-config/peerOrganizations/braketester.br/users/Admin@braketester.br/msp
    export CORE_PEER_ADDRESS=peer0.braketester.br:9051
...
parsePeerConnectionParameters() {
  ...
  while [ "$#" -gt 0 ]; do
    ...
    if [ $1 -eq 1 ]; then
      ...
      setGlobals braketester.br
      PEER="peer0.braketester.br"
    fi
    ## Set peer addresses
    PEERS="$PEERS $PEER"
